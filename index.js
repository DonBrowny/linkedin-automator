...
module.exports = async function processLineByLine(fileName) {
  if (!fileName) {
    return;
  }
  const { userName, password, connectionsPath, chromiumPath } = JSON.parse(
  ...
  const fileStream = fs.createReadStream(connectionsPath);
